...
- 27~30점: 교육용역이 핵심이거나 보안교육·개발보안·시큐어코딩·보안인식 제고가 핵심이다. 예: 클라우드·보안·AI·개발 장기교육, KDT, 정보보호 인재양성, 개발보안 교육, 보안인식 제고 교육
...
- 보안과 관련이 아주 깊은 사업은 교육이 없더라도 낮게 평가하지 않는다. 단, 순수 진단/컨설팅이면 교육 결합 공고보다 한 단계 낮게 둔다.
...
- 데이터 구축, 데이터 개방, 데이터 분석모델 개발, 플랫폼 구축, 앱·웹 개발, 포털 구축이 핵심이면 최종 등급은 반드시 `C` 이하로 둔다.
